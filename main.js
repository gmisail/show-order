...
let rename_episode = async (file, episode) => {
  const season_num =
    episode.season <= 9 ? `0${episode.season}` : `${episode.season}`;
  const episode_num =
    episode.episode <= 9 ? `0${episode.episode}` : `${episode.episode}`;

  const updated_name = `S${season_num}E${episode_num} - ${episode.title}`;

  console.log(
    `[show-order] rename ${file.file_name} -> ${updated_name}.${file.extension}`
  );

  fs.renameSync(
    `${file.path}${file.file_name}`,
    `${file.path}${updated_name}.${file.extension}`
  );
};

let process_episode = (finder, series_table, episode) => {
  const episode_name = readline
    .question(
      `[show-order] what is the episode name for ${episode.file_name}? `
    )
    .trim();
...
let run = async () => {
  ...
  episodes.forEach((ep) => process_episode(finder, series_table, ep));
